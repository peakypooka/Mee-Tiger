...
MULTI_TUNE_ITER = 25    # Optuna trials per round
...
plt.savefig(plot_path_holdout, dpi=200, bbox_inches='tight')
plt.close(fig)
...
try:
    if test_path and isinstance(test_path, str):
        ...
        # Rebuild lag/rolling features based on the test file's sampling step
        step_minutes_test = int(round(test_df["Date"].diff().dt.total_seconds().dropna().median() / 60.0))
        if step_minutes_test < 1:
            step_minutes_test = 1
        H_STEPS_TEST = max(1, FORECAST_MINUTES // step_minutes_test)

        loop_cols_test = [c for c in test_df.columns if "Loop" in c]
        if ADD_LOOP_LAGS:
            for c in loop_cols_test:
                for m in LAG_MINUTES:
                    s = max(1, m // step_minutes_test)
                    test_df[f"{c}_lag{s}"] = test_df[c].shift(s)
                for m in ROLL_MINUTES:
                    w = max(2, m // step_minutes_test)
                    test_df[f"{c}_roll{w}"] = test_df[c].shift(1).rolling(w, min_periods=max(2, w//2)).mean()
        if DROP_CURRENT_LOOP:
            test_df = test_df.drop(columns=loop_cols_test, errors="ignore")

        # Drop always-excluded columns if present
        for c in ["Plant - Power (kW)", "Plant - Efficiency (kW/Ton)", ORIG_TCOL]:
            if c in test_df.columns:
                test_df = test_df.drop(columns=[c])

        # If you want test metrics, create the same shifted target (if the file contains the original target)
        if ORIG_TCOL in load_data(test_path).columns:
            tmp = load_data(test_path)
            tmp["Date"] = pd.to_datetime(tmp["Date"], errors="coerce")
            tmp = tmp.sort_values("Date").reset_index(drop=True)
            tmp[ORIG_TCOL] = pd.to_numeric(tmp[ORIG_TCOL], errors="coerce")
            tmp["target_t_plus"] = tmp[ORIG_TCOL].shift(-H_STEPS_TEST)
            # align on Date
            test_df = test_df.merge(tmp[["Date", "target_t_plus"]], on="Date", how="left")
        ...
        if "target_t_plus" in test_df.columns and test_df["target_t_plus"].notna().any():
            ...
            plt.savefig(plot_path_ext, dpi=200, bbox_inches='tight')
            plt.close(fig_ext)
